...
impl PaillierPublic{
    pub fn verify_range_proof(paillier_context: &PaillierPublic, challenge: &ChallengeBits, encrypted_pairs: &EncryptedPairs, proof: &Proof ) -> bool{
        Paillier::verifier(&paillier_context.ek, &challenge, &encrypted_pairs, &proof, &EC::get_q(), RawCiphertext::from(&paillier_context.encrypted_secret_share)).is_ok()

    }
    pub fn generate_correct_key_challenge(paillier_context :&PaillierPublic) ->(Challenge, VerificationAid){
        let (challenge, verification_aid) = Paillier::challenge(&paillier_context.ek);
        (challenge, verification_aid)
    }

    pub fn verify_correct_key(proof: &CorrectKeyProof, aid: &VerificationAid) -> Result<(), CorrectKeyProofError>{
...
impl PartialSig{
    pub fn compute(ec_context: &EC, ek: &EncryptionKey, encrypted_secret_share: &BigInt, local_share: &KeyGenFirstMsg, ephemeral_local_share: &KeyGenFirstMsg, ephemeral_other_share: &party_one::KeyGenFirstMsg,  message: &BigInt) -> PartialSig{
        //compute R = k2* R1
        ...
        let c2 = Paillier::mul(ek, RawCiphertext::from(encrypted_secret_share), RawPlaintext::from(v) );
        //c3:
        PartialSig{
